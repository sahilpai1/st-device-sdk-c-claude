...
void TC_iot_update_dip_from_server_type_all_branches(void **state)
{
    ...
    // When/Then: PROD branches use prod_dip when populated
    context->devconf.prod_dip = &prod_dip;
    iot_update_dip_from_server_type(context, IOT_SERVER_PROD_AP_NORTH_EAST2);
    assert_int_equal(context->devconf.dip->dip_major_version, 1);
    iot_update_dip_from_server_type(context, IOT_SERVER_PROD_US_EAST1);
    iot_update_dip_from_server_type(context, IOT_SERVER_PROD_EU_WEST1);

    // When: prod_dip is NULL, falls into the "use default" branch
    context->devconf.prod_dip = NULL;
    iot_update_dip_from_server_type(context, IOT_SERVER_PROD_US_EAST1);

    // When/Then: ACC server type with and without acc_dip
    context->devconf.acc_dip = &acc_dip;
    iot_update_dip_from_server_type(context, IOT_SERVER_ACC_US_EAST2);
    assert_int_equal(context->devconf.dip->dip_major_version, 3);
    context->devconf.acc_dip = NULL;
    iot_update_dip_from_server_type(context, IOT_SERVER_ACC_US_EAST2);

    // When/Then: STG server type with and without stg_dip
    context->devconf.stg_dip = &stg_dip;
    iot_update_dip_from_server_type(context, IOT_SERVER_STG_US_EAST1);
    assert_int_equal(context->devconf.dip->dip_major_version, 5);
    context->devconf.stg_dip = NULL;
    iot_update_dip_from_server_type(context, IOT_SERVER_STG_US_EAST1);

    // When/Then: DEV server type with and without dev_dip
    context->devconf.dev_dip = &dev_dip;
    iot_update_dip_from_server_type(context, IOT_SERVER_DEV_US_EAST1);
    assert_int_equal(context->devconf.dip->dip_major_version, 7);
    context->devconf.dev_dip = NULL;
    iot_update_dip_from_server_type(context, IOT_SERVER_DEV_US_EAST1);

    // When: unknown server type takes the default branch
    iot_update_dip_from_server_type(context, IOT_SERVER_UNKNOWN);
