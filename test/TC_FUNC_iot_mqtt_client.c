...
/*
 * st_mqtt_destroy negative tests
 */
...
/*
 * st_mqtt_subscribe tests (negative paths only — positive requires a fully
 * mocked network round-trip that is already exercised via publish).
 */
...
/*
 * st_mqtt_unsubscribe negative tests
 */
...
/*
 * st_mqtt_publish_async negative tests
 */
...
/*
 * st_mqtt_publish negative tests
 */
...
/*
 * st_mqtt_change_ping_period tests
 */
...
/*
 * st_mqtt_yield tests (zero-time, disconnected client)
 */
...
/*
 * st_mqtt_connect negative tests that exit before the network flow
 */
...
/*
 * Feed an incoming PUBLISH QoS0 packet and drive st_mqtt_yield so the client
 * delivers it to the user callback via _iot_mqtt_deliver_publish.
 */
...
/*
 * PUBLISH QoS1 reception must trigger a PUBACK write on the network.
 */
...
/*
 * Duplicate PUBLISH (DUP bit set) must be discarded without invoking the
 * user callback.
 */
...
/*
 * An unknown MQTT packet type must be discarded silently.
 */
...
/*
 * A socket that port_net_read_poll reports as broken must tear the
 * connection down and emit an ST_MQTT_EVENT_DISCONNECTED to the user
 * callback.
 */
...
/*
 * st_mqtt_subscribe should succeed when the broker replies with SUBACK
 * granting the requested QoS.
 */
...
/*
 * A SUBACK with qos=2 should be accepted.  This exercises the success arm of
 * _iot_mqtt_process_received_ack for SUBACK.
 */
...
/*
 * st_mqtt_unsubscribe success path with UNSUBACK.
 */
...
/*
 * When the network write fails for a publish, the client must report failure.
 */
...
/*
 * st_mqtt_yield with a non-zero time keeps polling the run_cycle until the
 * timer expires.  This mainly covers the timer-path branch of st_mqtt_yield.
 */
...
/*
 * Exercise st_mqtt_change_ping_period while the client has active timers
 * (branches that delete and re-create the ping timers).
 */
...
/*
 * Disconnect on an already disconnected client must tear cleanly without
...
/*
 * Publish with QoS 0 on a connected client exercises the fire-and-forget
 * branch (no ack is expected back).
 */
...
/*
 * publish_async with a connected client drives the push path but does not
 * wait for the ack.
 */
void TC_st_mqtt_publish_async_connected(void **state)
{
    ...
    set_mock_port_net_write_skip_buf_check(1);
    set_mock_port_net_write_skip_len_check(1);
    rc = st_mqtt_publish_async(client, &msg);
    (void)rc;
    reset_mock_port_net_write_skip_flags();
...
/*
 * Subscribe failure when the network write fails.
 */
...
/*
 * Unsubscribe failure when the network write fails.
 */
