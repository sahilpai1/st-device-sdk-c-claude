...
/* ==== Additional broker URL/port branch coverage ==== */

void TC_STATIC_iot_es_set_broker_url_port_apnortheast2(void **state)
{
    st_mqtt_broker_info_t broker_info = {0};

    // When
    iot_error_t result = _iot_es_set_broker_url_port(SERVER_TYPE_AP_NORTH_EAST2, &broker_info);
    // Then
    assert_int_equal(result, IOT_ERROR_NONE);
    assert_non_null(broker_info.url);
    assert_int_equal(broker_info.port, 8883);
}

void TC_STATIC_iot_es_set_broker_url_port_useast1(void **state)
{
    st_mqtt_broker_info_t broker_info = {0};

    // When
    iot_error_t result = _iot_es_set_broker_url_port(SERVER_TYPE_US_EAST1, &broker_info);
    // Then
    assert_int_equal(result, IOT_ERROR_NONE);
    assert_non_null(broker_info.url);
    assert_int_equal(broker_info.port, 8883);
...
void TC_STATIC_iot_es_set_broker_url_port_euwest1(void **state)
{
    st_mqtt_broker_info_t broker_info = {0};

    // When
    iot_error_t result = _iot_es_set_broker_url_port(SERVER_TYPE_EU_WEST1, &broker_info);
    // Then
    assert_int_equal(result, IOT_ERROR_NONE);
    assert_non_null(broker_info.url);
    assert_int_equal(broker_info.port, 8883);
...
/* ==== iot_es_disconnect: COMMUNICATION variant with mqtt + topics set ==== */

void TC_STATIC_iot_es_disconnect_communication_with_topics(void **state)
{
    struct iot_context *context;
...
void TC_STATIC_iot_es_disconnect_registration_with_mqtt(void **state)
{
    struct iot_context *context;
...
/* ==== iot_update_dip: success path ==== */

void TC_STATIC_iot_update_dip_publish_failure(void **state)
{
    struct iot_context *context;
...
/* ==== _iot_mqtt_signin_client_callback branches ==== */

extern void _iot_mqtt_signin_client_callback(st_mqtt_event event, void *event_data, void *user_data);

void TC_STATIC_iot_mqtt_signin_client_callback_msg_pre_success(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
...
void TC_STATIC_iot_mqtt_signin_client_callback_msg_command_topic_self(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
...
void TC_STATIC_iot_mqtt_signin_client_callback_msg_command_topic_unknown(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
    char payload[] = "{\"commands\":[]}";
...
void TC_STATIC_iot_mqtt_signin_client_callback_msg_notification_topic(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
...
void TC_STATIC_iot_mqtt_signin_client_callback_msg_unknown_topic(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
    char payload[] = "{}";
...
void TC_STATIC_iot_mqtt_signin_client_callback_publish_failed_no_seq(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
    char payload[] = "no-sequence-number-here";

    context = (struct iot_context *)calloc(1, sizeof(struct iot_context));
    msg.payload = payload;
    msg.payloadlen = strlen(payload);

    // When: publish-failed with unparseable payload -> _iot_parse_sequence_num returns -1, branch breaks
    _iot_mqtt_signin_client_callback(ST_MQTT_EVENT_PUBLISH_FAILED, &msg, context);

    free(context);
}

void TC_STATIC_iot_mqtt_signin_client_callback_publish_timeout_with_seq(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg = {0};
    char payload[] =
        "{\"deviceEvents\":[{\"providerData\":{\"sequenceNumber\":42,\"timestamp\":\"123\",\"stateChange\":\"Y\"}}]}";
    ...
    // When: publish-timeout with parseable sequence number -> calls iot_command_send
    _iot_mqtt_signin_client_callback(ST_MQTT_EVENT_PUBLISH_TIMEOUT, &msg, context);

    /* Teardown: drain the queued command */
    {
...
void TC_STATIC_iot_mqtt_signin_client_callback_disconnected_ping_fail(void **state)
{
    struct iot_context *context;
...
void TC_STATIC_iot_mqtt_signin_client_callback_disconnected_ping_timeout(void **state)
{
    struct iot_context *context;
...
void TC_STATIC_iot_mqtt_signin_client_callback_unknown_event(void **state)
{
    struct iot_context *context;
...
/* ==== mqtt_reg_sub_cb branches via _iot_mqtt_registration_client_callback ==== */

void TC_STATIC_iot_mqtt_registration_client_callback_with_status_cb(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
    char *reg_payload =
...
void TC_STATIC_iot_mqtt_registration_client_callback_expired_jwt_branch(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
void TC_STATIC_iot_mqtt_registration_client_callback_error_event_in_payload(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
void TC_STATIC_iot_mqtt_registration_client_callback_unknown_event_in_payload(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
void TC_STATIC_iot_mqtt_registration_client_callback_dip_missing_id(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
void TC_STATIC_iot_mqtt_registration_client_callback_dip_missing_major(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
void TC_STATIC_iot_mqtt_registration_client_callback_dip_missing_minor(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
void TC_STATIC_iot_mqtt_registration_client_callback_invalid_location(void **state)
{
    struct iot_context *context;
    st_mqtt_msg msg;
...
/* ==== iot_es_connect: drive deeper paths via a fully-populated context ==== */
...
void TC_STATIC_iot_es_connect_registration_no_broker(void **state)
{
    struct iot_context *context;
    iot_error_t result;
    ...
    context->work_queue_signal = iot_os_eventgroup_create();
    context->devconf.mnid = REG_TEST_MNID;
    context->server_type = SERVER_TYPE_UNKNOWN; /* forces the "url does not exist" branch */
...
void TC_STATIC_iot_es_connect_registration_unknown_server(void **state)
{
    struct iot_context *context;
    iot_error_t result;
    iot_error_t err;
    ...
    context->server_type = (st_server_type)(SERVER_TYPE_EU_WEST1 + 1);
...
void TC_STATIC_iot_es_connect_communication_no_reg(void **state)
{
    struct iot_context *context;
    iot_error_t err;
...
void TC_STATIC_iot_es_connect_communication_with_reg_no_broker(void **state)
{
    struct iot_context *context;
    iot_error_t err;
    ...
    context->server_type = SERVER_TYPE_UNKNOWN;
...
void TC_STATIC_iot_update_dip_publish_path(void **state)
{
    struct iot_context *context;
    st_mqtt_client mqtt_cli;
...
/* ==== Successful mqtt connect path with mocked CONNACK ==== */
